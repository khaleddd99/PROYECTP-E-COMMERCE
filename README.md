...
import functools # Necesario para la función reduce
import datetime # Para la fecha del pedido

# --- 1. Definición de Estructuras de Datos (Inmutables) ---
...
# --- NUEVO: Estructuras para el Pedido ---
# El ItemPedido es igual al ItemCarrito, pero podemos
# añadir más datos como el precio_congelado en el momento de la compra
class ItemPedido(BaseModel):
    model_config = ConfigDict(frozen=True)
    producto_id: int
    cantidad: int
    precio_congelado: float # Precio al momento de la compra
...
class Pedido(BaseModel):
    ...
    total_pedido: float
    fecha: str
    estado: str # ej: 'Pendiente', 'Completado'


# --- 2. Simulación de Base de Datos (Estado Global) ---
...
# --- NUEVO: Base de datos de Pedidos ---
...
# --- 4. Lógica Funcional Pura (Módulo 1: Productos) ---
...
# --- 5. Lógica Funcional Pura (Módulo 2: Carrito) ---
...
# --- 6. NUEVO: Lógica Funcional Pura (Módulo 3: Pedidos) ---
...
def fn_descontar_stock_de_pedido(
    ...
    # La función 'reducer' que se aplica en cada paso
    def reducer(
        productos_acumulados: List[Producto], # La lista de productos del paso anterior
        item_actual: ItemCarrito             # El item del carrito que estamos procesando
    ) -> List[Producto]:
        # Devuelve una *nueva* lista con el stock actualizado para este item
    ...
    # functools.reduce(función_reductora, secuencia_a_reducir, valor_inicial)
    nueva_lista_productos = functools.reduce(
        reducer,
        carrito.items,
        lista_productos # El valor inicial es la lista de productos original
    )
...
# --- 7. Endpoints de la API (Módulo 1 y 2) ---
# (Se omiten por brevedad, son los mismos que ya tienes)
...
# --- 8. NUEVO: Endpoints de la API (Módulo 3: Pedidos) ---
...
async def procesar_checkout_carrito(carrito_id: int):
    ...
    """
    # Declaramos globales porque vamos a modificar TODOS los estados
    global db_productos
    global db_carritos
    global db_pedidos
    ...
    # 2. (Validación de Stock) - Opcional pero recomendado
    # ... (aquí iría una lógica para verificar stock antes de descontar)

    # 3. Aplicar las transformaciones funcionales
    
    # 3.1. Descontar el stock de la lista de productos
    nueva_lista_productos = fn_descontar_stock_de_pedido(db_productos, carrito)
    
    # 3.2. Crear el nuevo objeto Pedido
    nuevo_id = len(db_pedidos) + 1
    nuevo_pedido = fn_convertir_carrito_a_pedido(carrito, db_productos, nuevo_id)
    
    # 3.3. Crear la nueva lista de pedidos
    nueva_lista_pedidos = fn_agregar_pedido_a_db(db_pedidos, nuevo_pedido)
    
    # 3.4. Obtener una versión limpia del carrito
    carrito_limpiado = fn_limpiar_carrito(carrito)
    
    # 3.5. Crear la nueva lista de carritos (con el carrito ya limpio)
    nueva_lista_carritos = fn_actualizar_carrito_en_db(db_carritos, carrito_limpiado)
    
    # 4. Mutar el estado global (el "commit" final)
...
# --- 9. Punto de entrada para ejecutar la app ---
